<template>
  <Disclosure as="nav" class="mt-1 bg-white" v-slot="{ open }">
      ...
      <div class="relative flex h-16 items-center justify-between">
        <div class="absolute inset-y-0 left-0 flex items-center sm:hidden">
          ...
          <DisclosureButton class="relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-gray-700 hover:text-white focus:ring-2 focus:ring-white focus:outline-hidden focus:ring-inset">
            ...
            <Bars3Icon v-if="!open" class="block size-6 text-black" aria-hidden="true" />
            <XMarkIcon v-else class="block size-6" aria-hidden="true" />
          </DisclosureButton>
        </div>
        <div class="flex flex-1 items-center justify-left sm:items-stretch sm:justify-start">
          <div class="flex shrink-0 items-center ml-10">
            <img class="h-6 w-auto" src="https://www.gstatic.com/images/branding/googlelogo/svg/googlelogo_clr_74x24px.svg?color=indigo&shade=500" alt="Your Company" />
            <h1 class="text-2xl pl-1 text-gray-500">StockWise</h1>
          </div>
